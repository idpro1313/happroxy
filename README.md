...
├── config/
│   └── happ-routing.json # шаблон split-tunnel для Happ
└── scripts/
    ├── install.sh
...
> Сертификаты лежат в `./cert/` на хосте (= `/root/cert/` в контейнере).
...
Архивы: `backups/happroxy_YYYYMMDD_HHMMSS.tar.gz`

**Восстановление:**

```bash
docker compose down
tar -xzf backups/happroxy_YYYYMMDD_HHMMSS.tar.gz -C .
docker compose up -d
```
