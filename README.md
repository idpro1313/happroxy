...
**Что происходит:** при `GlobalProxy: true` в правилах маршрутизации **весь** трафик идёт через VPN. Если туннель не работает — интернет **полностью** пропадает (это не баг Windows, а логика Happ).
...
3. Подключитесь к серверу **Shadowsocks (порт 8388)**, не Hysteria2.
4. Если SS работает — интернет есть; потом снова добавьте routing через `generate-routing-deeplink.sh`.
...
| Подписка с `127.0.0.1` | Исправить **URI обратного прокси** |
