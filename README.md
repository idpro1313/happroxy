...
| **Прослушивание IP** | Публичный IP VM (`SERVER_IP` из `.env`) |
| **Порт подписки** | `38471` (тот же, что порт панели) |
| **URI-путь** | `/sub/family/` (или значение `SUB_PATH` + `/` в конце) |
...
| Приватный ключ | `/root/cert/selfsigned.key` |
...
Если Trojan не нужен, установите `ENABLE_TROJAN=false` в `.env` и не создавайте подключение.
...
| Проблема | Решение |
|---|---|
| Подписка с `127.0.0.1` | Задайте **Прослушивание IP** = публичный IP в **Настройки панели → Подписка** |
